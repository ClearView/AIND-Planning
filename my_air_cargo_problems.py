...
class AirCargoProblem(Problem):
    ...
    def h_ignore_preconditions(self, node: Node):
        ...
        #while not solved:
        #    kb = PropKB()
        #    kb.tell(decode_state(new_state, self.state_map).sentence())
        #    for a in self.actions_list:
        #        changed = False
        #        for eff in a.effect_add:
        #            if eff in self.goal:
        #                kb.tell(eff)
        #                changed = True
        #        for eff in a.effect_rem:
        #            if eff not in self.goal:
        #                kb.retract(eff)
        #                changed = True
        #        if changed:
        #            count += 1
        #            break
        #    new_state = encode_state(FluentState(kb.clauses, []), self.state_map)
        #    solved = self.goal_test(new_state)
        #    print(solved)
        #print(count)
        #return count
